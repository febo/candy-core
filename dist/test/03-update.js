...
(0, tape_1.default)('update', async (t) => {
    ...
    const { tx: updateTransaction1 } = await init.updateCandyMachine(t, address, payerPair, data, fstTxHandler);
    ...
    const { tx: updateTransaction2 } = await init.updateCandyMachine(t, address, payerPair, data, fstTxHandler);
    await updateTransaction2.assertError(t);
    data.itemsAvailable = 100;
    data.configLineSettings.nameLength = 5;
    data.configLineSettings.uriLength = 10;
    const { tx: updateTransaction3 } = await init.updateCandyMachine(t, address, payerPair, data, fstTxHandler);
...
(0, tape_1.default)('update (hidden settings)', async (t) => {
    ...
    const { tx: updateTransaction2 } = await init.updateCandyMachine(t, address, payerPair, updatedData, fstTxHandler);
    await updateTransaction2.assertError(t);
});
